...
class Order(TypedDict):
    ...
    quantity: float
    price: float
    order_type: Optional[Literal["MARKET", "LIMIT", "SL-M", "SL"]]
    product: Optional[Literal["MIS", "CNC", "NRML", "CO"]]
...
class KiteConnect(object):
    ...
    def set_session_expiry_hook(self, method: Callable[[]]):
        """
    ...
    def generate_session(self, request_token: str, api_secret: str):
        ...
        if resp["login_time"] and len(resp["login_time"]) == 19:
            resp["login_time"] = dateutil_parse(resp["login_time"])

        return resp

    def invalidate_access_token(self, access_token: Optional[str] = None):
        """
    ...
    def margins(self, segment: Optional[Literal["equity", "commodity"]] = None):
    ...
    def place_order(
        ...
        exchange: Literal["NSE", "BSE", "NFO", "CDS", "BFO", "MCX", "BCD"],
        tradingsymbol: str,
        transaction_type: Literal["BUY", "SELL"],
        quantity: int,
        product: Literal["MIS", "CNC", "NRML", "CO"],
        order_type: Literal["MARKET", "LIMIT", "SL-M", "SL"],
    ...
    def modify_order(
        ...
        price: Optional[float] = None,
        order_type: Optional[Literal["MARKET", "LIMIT", "SL-M", "SL"]] = None,
        trigger_price: Optional[float] = None,
        validity: Optional[Literal["DAY", "IOC", "TTL"]] = None,
        validity_ttl: Optional[int] = None,
        disclosed_quantity: Optional[int] = None,
    ...
    def _format_response(self, data: Union[List[Dict[str, Any]], Dict[str, Any]]):
        ...
        for item in _list:
            ...
            for field in [
                ...
                if item.get(field) and len(item[field]) == 19:
                    item[field] = dateutil_parse(item[field])
    ...
    def convert_position(
        ...
        tradingsymbol,
        transaction_type: Literal["BUY", "SELL"],
        position_type: Literal["day", "overnight"],
        quantity: int,
        old_product: Literal["MIS", "CNC", "NRML", "CO"],
        new_product: Literal["MIS", "CNC", "NRML", "CO"],
    ):
    ...
    def mf_orders(self, order_id: Optional[str] = None):
    ...
    def mf_sips(self, sip_id: Optional[str] = None):
    ...
    def modify_mf_sip(
        ...
        sip_id: str,
        amount: Optional[float] = None,
        status: Optional[Literal["COMPLETED", "CANCELLED", "PENDING"]] = None,
        instalments: Optional[float] = None,
    ...
    def cancel_mf_sip(self, sip_id: str):
    ...
    def instruments(
        self,
        exchange: Optional[
            Literal["NSE", "BSE", "NFO", "CDS", "BFO", "MCX", "BCD"]
        ] = None,
    ):
        """
    ...
    def ltp(self, *instruments):
        ...
        # If first element is a list then accept it as instruments list for legacy reason
        if isinstance(instruments[0], list) and len(instruments) > 0:
            ins = instruments[0]
    ...
    def _format_historical(self, data: Dict[str, Any]):
        records = []
        for d in data["candles"]:
            record = {
                "date": dateutil_parse(d[0]),
                "open": d[1],
                "high": d[2],
                "low": d[3],
                "close": d[4],
    ...
    def trigger_range(self, transaction_type: Literal["BUY", "SELL"], *instruments):
    ...
    def get_gtt(self, trigger_id: str):
    ...
    def _get_gtt_payload(
        ...
        tradingsymbol: str,
        exchange: Literal["NSE", "BSE", "NFO", "CDS", "BFO", "MCX", "BCD"],
        trigger_values: List[Dict[str, float]],
        last_price: float,
        orders: List[Order],
    ):
        """Get GTT payload"""
    ...
    def order_margins(self, params: List[Dict[str, Any]]):
        """
    ...
    def basket_order_margins(
        ...
        mode: Optional[Literal["compact", "full"]] = None,
    ):
        """
    ...
    def _parse_instruments(self, data):
        ...
        for row in reader:
            ...
            if len(row["expiry"]) == 10:
                row["expiry"] = dateutil_parse(row["expiry"]).date()
    ...
    def _parse_mf_instruments(self, data):
        ...
        for row in reader:
            ...
            if len(row["last_price_date"]) == 10:
                row["last_price_date"] = dateutil_parse(row["last_price_date"]).date()
    ...
    def _get(
        ...
        if not isinstance(resp, dict):
            raise ex.DataException("The data fetched is not of the correct data type")
        return resp
    ...
    def _request(
        ...
        *,
        ...
        is_json: bool = False,
        query_params: Optional[Union[Dict[str, str], List[Dict[str, str]]]] = None,
    ):
        ...
        if "json" in r.headers["content-type"]:
            ...
            return r.content
        raise ex.DataException(
            f"Unknown Content-Type ({r.headers['content-type']}) with response: ({r.content})"
        )
